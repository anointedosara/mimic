...
import { Users, Skull, MessagesSquare, Play, Bot, Gauge, UserRound, Trophy } from "lucide-react";
...
import { MAX_PLAYERS, maxImposters } from "@/lib/game/config";
import { cn } from "@/lib/utils";
import { playSound } from "@/lib/sounds";
import { makeId } from "@/lib/passplay/storage";
import { AI_NAMES } from "@/lib/solo/names";
import type { Difficulty } from "@/lib/solo/types";
import type { TournamentPlayer, TournamentSettings } from "@/lib/tournament/types";
import { Leaderboard } from "./leaderboard";

/** Tournaments need enough seats to run several elimination rounds. */
const TOURNAMENT_MIN_PLAYERS = 4;
const HUMAN_KEY = "mimic:solo:human"; // shared identity with Solo Mode

const DIFFICULTIES: Array<{ id: Difficulty; label: string; emoji: string; blurb: string }> = [
  { id: "easy", label: "Easy", emoji: "🌱", blurb: "Makes mistakes, votes loosely, gives obvious clues." },
  { id: "medium", label: "Medium", emoji: "⚖️", blurb: "Usually reasonable — but occasionally gets tricked." },
  { id: "hard", label: "Hard", emoji: "🔥", blurb: "Remembers clues, spots the blenders, accuses smartly." },
  { id: "chaos", label: "Chaos", emoji: "🌀", blurb: "Exaggerated personalities, wild alliances, anything goes." },
];
...
export function Setup({
  ...
  onStart: (human: TournamentPlayer, settings: TournamentSettings) => void;
  ...
  const maxRounds = total - imposters; // rough: normals that can be voted off before parity

  function start() {
    if (!canStart) return;
    saveHuman(trimmedName, avatar);
    const human: TournamentPlayer = { id: makeId(), name: trimmedName, avatar, isAI: false, personality: null };
    onStart(human, { totalPlayers: total, imposterCount: imposters, clueRounds, difficulty });
  }
    ...
    <div className="mx-auto max-w-2xl">
      <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }}>
        <div className="flex flex-wrap items-center gap-2">
          <h1 className="font-display text-3xl font-black sm:text-4xl">Survival Tournament</h1>
          <Badge variant="gold" className="gap-1">
            <Trophy className="h-3 w-3" /> Last one standing
          </Badge>
        </div>
        <p className="mt-1 text-muted-foreground">
          One vote-off per round. The imposters hide, the table hunts, and losers become spectators —
          until one side is left standing. Part party game, part reality show.
        </p>
      </motion.div>
      ...
      <Card className="mt-6">
        <CardHeader>
          ...
          <CardDescription>Pick your name and avatar. Everyone else is AI.</CardDescription>
        </CardHeader>
      ...
      <Card className="mt-4">
        <CardHeader>
          ...
          <CardDescription>
            You plus {total - 1} AI. Up to ~{Math.max(1, maxRounds)} elimination round
            {maxRounds === 1 ? "" : "s"} before a side wins.
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          <div className="flex items-center gap-4">
            <span className="text-xs font-bold text-muted-foreground">{TOURNAMENT_MIN_PLAYERS}</span>
            <Slider
      ...
      <Card className="mt-4">
        <CardHeader>
          ...
          <CardDescription>
            1 to {impMax} imposter{impMax > 1 ? "s" : ""}. They stay imposters every round until caught.
          </CardDescription>
        </CardHeader>
        <CardContent>
          <div className="flex flex-wrap gap-2">
      ...
      <Card className="mt-4">
        <CardHeader>
          ...
          <CardDescription>How sharp — and how wild — your rivals play.</CardDescription>
        </CardHeader>
      ...
      <Card className="mt-4">
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
            <MessagesSquare className="h-5 w-5 text-mimic-amber" /> Clue passes
          </CardTitle>
          <CardDescription>
            How many times the table goes around giving clues before each vote.
          </CardDescription>
        </CardHeader>
        <CardContent>
          <div className="flex flex-wrap gap-2">
      ...
      <div className="sticky bottom-4 z-10 mt-6 flex flex-col items-center justify-between gap-3 rounded-2xl glass-strong p-4 sm:flex-row">
        <div className="text-sm text-muted-foreground">
          {total} players · {imposters} imposter{imposters > 1 ? "s" : ""} · {difficulty} · {clueRounds}{" "}
          pass{clueRounds > 1 ? "es" : ""}
        </div>
        <Button
          variant="gradient"
          size="lg"
          ...
          disabled={!canStart}
        >
          <Play className="h-5 w-5" /> {canStart ? "Start tournament" : "Enter your name"}
        </Button>
      </div>
    </div>
  );
