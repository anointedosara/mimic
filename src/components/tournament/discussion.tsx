...
import { ConversationFeed, ChatComposer, PlayerBoard } from "./conversation";
...
export function Discussion() {
  ...
  const turnIndex = useTournamentStore((s) => s.turnIndex);
  const totalClueRounds = useTournamentStore((s) => s.totalClueRounds);
  const awaitingHuman = useTournamentStore((s) => s.awaitingHuman);
  const submitHumanClue = useTournamentStore((s) => s.submitHumanClue);
  const startVoting = useTournamentStore((s) => s.startVoting);
  ...
  const totalTurns = active.length * totalClueRounds;
  const currentPass = Math.min(totalClueRounds, Math.floor(turnIndex / Math.max(1, active.length)) + 1);
  const currentPlayer = turnIndex < totalTurns ? active[turnIndex % active.length] : null;
  ...
  return (
    <div className="mx-auto max-w-2xl space-y-4">
      ...
      <div className="rounded-2xl glass p-4">
        <div className="flex items-center justify-between gap-3">
          <div className="min-w-0">
            <div className="flex items-center gap-2 font-display text-lg font-bold">
              <Lightbulb className="h-5 w-5 text-mimic-amber" /> Round {roundNumber} · Clues
            </div>
            <p className="text-sm text-muted-foreground">
              Pass {currentPass}/{totalClueRounds} ·{" "}
              {currentPlayer
        ...
        {!humanActive && (
          <div className="mt-3 inline-flex items-center gap-1.5 rounded-full bg-white/[0.05] px-2.5 py-1 text-xs text-muted-foreground">
            <Eye className="h-3.5 w-3.5" /> Spectating — you can watch and chat
          </div>
        )}
      </div>
      ...
      >
        <Vote className="h-5 w-5" /> Skip to the vote
      </Button>
    </div>
  );
}
